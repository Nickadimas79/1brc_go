...
func Fourth() {
	...
	if *executionProfile != "" {
		f, err := os.Create("./fourth/profiles/" + *executionProfile)
		if err != nil {
		...
		err = trace.Start(f)
		if err != nil {
			log.Println("error starting trace: ", err)
		}
		defer trace.Stop()
	}

	if *cpuProfile != "" {
		f, err := os.Create("./fourth/profiles/" + *cpuProfile)
		if err != nil {
	...
	if *memProfile != "" {
		f, err := os.Create("./fourth/profiles/" + *memProfile)
		if err != nil {
...
func readFileLineByLineIntoAMap(filepath string) (map[string]cityTempInfo, error) {
	...
	mapOfTemp := make(map[string]cityTempInfo)
	resultStream := make(chan map[string]cityTempInfo, 10)
	chunkStream := make(chan []byte, 15)
	chunkSize := 64 * 1024 * 1024
	var wg sync.WaitGroup

	// spawn workers to consume (process) file chunks
	...
	go func() {
		...
		// wait for all chunks to be processed before closing the result stream
		wg.Wait()
		close(resultStream)
	}()
...
func processReadChunk(buf []byte, resultStream chan<- map[string]cityTempInfo) {
	toSend := make(map[string]cityTempInfo)
	var start int
	var city string
